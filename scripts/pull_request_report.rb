...
def send_report_to_slack (**args)
    ...
    unless ($SLACK_BOT_TOKEN && $SLACK_CHANNEL_ID)
        w = "SLACK_BOT_TOKEN and SLACK_CHANNEL_ID are required to be configured for posting messages within the slack channel thread. Please ensure that the SLACK_BOT_TOKEN and SLACK_CHANNEL_ID are configured correctly."
        $logger.warn w
        $logger.close()
    else
        ...
        # Add thread reaction to the message that we posted.
